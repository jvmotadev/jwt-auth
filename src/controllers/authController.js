...
const dateProtectedContent = (request, response) => {
  const token = request.headers["authorization"];

  if (!token) {
    return response.status(403).json({ message: "Token não fornecido :P" });
  }
...
const textProtectedContent = (request, response) => {
  const token = request.headers["authorization"];
  ...
  try {
    ...
    response.json({ 
      status: "success",
      message: "Parabéns! Você conseguiu acessar o conteúdo protegido!"
  });
...
module.exports = { login, protectedContent, dateProtectedContent, textProtectedContent };
